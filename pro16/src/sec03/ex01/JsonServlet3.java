...
public class JsonServlet3 extends HttpServlet {
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		JSONObject totaObject = new JSONObject();//배열을 최종적으로 저장할  JSONObject 객체를 생성
		...
		membersArray.add(memberInfo);
		totaObject.put("members", membersArray);//회원 정보를 저장할 배열을 배열 이름 member로 totalObject에 저장

		JSONArray bookArray = new JSONArray();//JSONArray 객체에 생성
		...
		totaObject.put("books", bookArray);//도서 정보를 저장할 배열을 배열이름 books로 totalObject에 저장
