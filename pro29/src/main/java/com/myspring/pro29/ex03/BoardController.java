...
public class BoardController {
	...
	@RequestMapping(value = "/all", method = RequestMethod.GET)//Get 방식으로 요청하므로 모든 글의 정보를 조회
	...
	@RequestMapping(value = "/{articleNO}", method = RequestMethod.GET)//Get방식으로 요청하면서 글 번호를 전달하므로 글 번호에 대한 글 정보를 조회
	...
	@RequestMapping(value = "", method = RequestMethod.POST)//Post방식으로 요청하므로 요청 시 Json으로 전달되는 객체를 새 글로 추가함
	...
	public ResponseEntity<String> modArticle (@PathVariable("articleNO") Integer articleNO, @RequestBody ArticleVO articleVO) //전송된 json 회원 정보를 바로 ArticleNo 객체의 속성에 설정
